...
contract Domains is ERC721URIStorage {
  ...
  constructor(string memory _tld) payable ERC721("Resister-boy Name Service", "resister-boy") {
    TLD = _tld;
    console.log("%s name service deployed", _tld);
  }

  function price(string calldata name) public pure returns (uint) {
    uint len = StringUtils.strlen(name); // String length 반환하는 function
    require(len > 0);
    if(len == 3) {
      return 5 * 10 ** 14; 
    } else if (len == 4) {
  ...
  function register(string calldata name) public payable {
    ...
    uint256 _price = price(name); // name을 param으로 funciton price 실행 후 _price에 allocation

    require(msg.value >= _price, "Not enough Matic paid"); // check wallet balance can paid Matic
    ...
    string memory json = Base64.encode(
        abi.encodePacked('{"name": "', _name,'", "description": "A domain on the Ninja name service", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(finalSvg)),'","length":"', strLen, '"}')
      );
